...
export default function AdditionalTrainingsManager({
  ...
  const analytics = useMemo(() => {
    ...
    employees.forEach((emp) => {
      (emp.additionalTrainings || []).forEach((training, index) => {
    ...
    filtered.forEach((t) => {
      ...
      const emp = byEmployeeMap.get(t.employeeId);
      emp.trainings.push(t);
      emp.total += 1;
      if (t.expired) emp.expired += 1;
      else emp.valid += 1;
    });
  ...
  const uniqueTypes = useMemo(() => {
    ...
    employees.forEach((emp) => {
      (emp.additionalTrainings || []).forEach((t) => {
