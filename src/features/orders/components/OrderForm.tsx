...
import type { OrderInsert, OrderUpdate } from "@/entities/order";
...
interface OrderFormProps {
  ...
  onClose?: () => void;
  onSave?: (result: { isEdit: boolean; order: OrderRow | Partial<OrderRow> }) => Promise<void>;
...
const createDefaultFormState = (): OrderFormState => ({
  order_number: "",
  order_name: "",
...
const mapOrderToFormState = (order: OrderRow): OrderFormState => ({
...
export default function OrderForm({
  ...
  const selectedEmployees = useMemo(
    () =>
      form.responsible_persons
        ...
        .filter((e): e is RegistryEmployee => e !== undefined),
  ...
  const setFieldValue = useCallback(<K extends keyof OrderFormState>(field: K, value: OrderFormState[K]) => {
  ...
  const handleInputChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
      ...
      setFieldValue(name as keyof OrderFormState, value as OrderFormState[keyof OrderFormState]);
    },
    [setFieldValue]
  );

  const handleResponsiblePersonsChange = useCallback((nextPersons: string[]) => {
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        if (isEdit && order) {
          await updateOrder(order.id, payload as OrderUpdate);
        } else {
          await createOrder(payload as OrderInsert);
        }

        await onSave({
          isEdit,
          order: isEdit && order ? { ...order, ...payload } : payload,
        });
      } catch (error) {
