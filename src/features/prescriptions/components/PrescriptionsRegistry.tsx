...
import type { RegistryEmployee } from "@/features/prescriptions/services/prescriptionsService";
import {
...
type PrescriptionRow = {
  ...
  created_at?: string;
  responsible_person?: { name: string; profession?: string; id?: string; organization?: string } | null;
...
export default function PrescriptionsRegistry({ addNotification }: PrescriptionsRegistryProps) {
  ...
  const prescriptionsWithMeta = useMemo((): PrescriptionRow[] => {
    const today = new Date().toISOString().split("T")[0];

    return prescriptions.map((prescription) => {
      const responsiblePerson = prescription.responsible_person_id
        ? (employeesById.get(prescription.responsible_person_id) ?? null)
        : null;
  ...
  const filteredPrescriptions = useMemo((): PrescriptionRow[] => {
    ...
    return prescriptionsWithMeta.filter((prescription) => {
      ...
      if (statusFilter !== "all" && prescription.actual_status !== statusFilter) {
        return false;
      }

      if (organizationFilter !== "all" && prescription.organization !== organizationFilter) {
        return false;
      }
  ...
  const handleEditPrescription = useCallback((prescription: PrescriptionRow) => {
  ...
  const handleDeletePrescription = useCallback(
    async (prescriptionId: string) => {
      try {
        ...
        addNotification("Предписание удалено.", TOAST_TYPES.SUCCESS, TOAST_DURATION.NORMAL);
      } catch (error) {
        console.error("Ошибка удаления предписания:", error);
        addNotification("Не удалось удалить предписание.", TOAST_TYPES.ERROR, TOAST_DURATION.NORMAL);
      }
    },
