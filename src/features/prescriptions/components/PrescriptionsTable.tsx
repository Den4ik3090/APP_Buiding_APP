...
interface PrescriptionsTableProps {
  ...
  onEdit?: (prescription: PrescriptionRow) => void;
...
const getComparableValue = (prescription: PrescriptionRow, field: string): string | number => {
  switch (field) {
    case "issue_date":
    ...
    default:
      return (prescription as Record<string, unknown>)[field] as string || "";
  }
};

const comparePrescriptions = (left: PrescriptionRow, right: PrescriptionRow, field: string, direction: string) => {
...
const isWindowsFilePath = (value: string) => /^[a-zA-Z]:[\\/]/.test(value);
const isNetworkFilePath = (value: string) => /^\\\\/.test(value);

const resolveDocumentHref = (documentUrl: string | null | undefined): string => {
...
const getDownloadFileName = (documentUrl: string | null | undefined, prescriptionNumber: string | undefined): string => {
...
const StatusBadge = ({ status }: StatusBadgeProps) => {
  ...
  return (
    <span className={`prescription-status-badge ${config.className}`}>
      <Icon size={14} />
      <span>{PRESCRIPTION_STATUS_LABELS[status as PrescriptionStatusValue]}</span>
    </span>
  );
};

type DeadlineCellProps = { deadline: string | null | undefined; status?: string };

const DeadlineCell = ({ deadline, status }: DeadlineCellProps) => {
  if (!deadline) {
          ...
          {sortedPrescriptions.map((prescription) => {
            ...
            return (
              <tr key={prescription.id}>
                ...
                <td>
                  <StatusBadge status={prescription.actual_status || PRESCRIPTION_STATUSES.OPEN} />
                </td>
...
export default memo<PrescriptionsTableProps>(PrescriptionsTable);
