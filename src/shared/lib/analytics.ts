...
export function computeAnalytics(employees: any[], getDaysDifference: (date: string) => number) {
  const now = new Date();
  ...
  const rows = employees
    ...
    .map((e) => {
      ...
      const daysToExpire = Math.ceil((nextDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const avgDaysOverdue =
    overdueDays.length === 0
      ? 0
      : overdueDays.reduce((a: number, b: number) => a + b, 0) / overdueDays.length;
...
function groupCompliance(rows: any[], key: string) {
  const map = new Map<string, { name: string; total: number; valid: number; warning: number; expired: number }>();
  ...
  for (const r of rows) {
    ...
    item.total += 1;
    item[r.status as "valid" | "warning" | "expired"] += 1;
  }
