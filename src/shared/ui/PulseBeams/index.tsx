...
const GradientColors = ({ colors = defaultGradient }: { colors?: GradientColors }) => (
  <>
    ...
    {beams.map((beam, index) => (
      <React.Fragment key={index}>
        ...
        {beam.connectionPoints?.map((point, pi) => (
          <circle key={`${index}-${pi}`} cx={point.cx} cy={point.cy} r={point.r}
            fill={baseColor} stroke={accentColor} />
        ))}
      </React.Fragment>
    ))}
    ...
    <defs>
      {beams.map((beam, index) => (
        <motion.linearGradient
          ...
          // @ts-expect-error framer-motion SVG gradient transition is untyped
