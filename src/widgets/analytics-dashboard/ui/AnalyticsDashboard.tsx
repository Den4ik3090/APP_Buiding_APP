...
type DueSoonEmployee = EnrichedEmployee & {
...
export default function AnalyticsDashboard({
  ...
  const [openResponsible, setOpenResponsible] = useState<string | null>(null);
  ...
  const professionOptions = useMemo<string[]>(() => {
    const unique = new Set(enrichedEmployees.map((emp) => emp.professionLabel));
    return Array.from(unique).sort((a, b) => a.localeCompare(b));
  }, [enrichedEmployees]);

  const filteredEmployees = useMemo<EnrichedEmployee[]>(() => {
  ...
  const responsibleGroups = useMemo<ResponsibleGroup[]>(() => {
    const collector = new Map<string, EnrichedEmployee[]>();
  ...
  const managerPieData = useMemo<PieDataItem[]>(() => {
    if (!responsibleGroups.length) return [{ name: "Нет данных", value: 1 }];
    const top: PieDataItem[] = responsibleGroups.slice(0, 10).map((group) => ({
      name: group.name,
      value: group.count,
    }));
  ...
  const professionBarData = useMemo<BarDataItem[]>(() => {
  ...
  const dueSoonList = useMemo<DueSoonEmployee[]>(
    () =>
      filteredEmployees
        .filter((emp) => emp.daysSinceTraining !== null)
        .map((emp) => ({
          ...emp,
          daysUntilRefresh: DAYS_THRESHOLD - (emp.daysSinceTraining as number),
        }))
  ...
  const activeGroup = useMemo<ResponsibleGroup | null>(
    () =>
  ...
  const handleEmployeeClick = useCallback((employee: EnrichedEmployee) => {
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip
                  ...
                  formatter={(value, name) => [`${value} чел.`, String(name)]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
